...
export function createAuthModule(app: Express): void {
  ...
  app.post("/api/auth/discord", async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { code } = req.body;
      ...
      if (!tokenResponse.ok) {
        throw new AppError(401, "Failed to authenticate with Discord");
      }
      ...
      if (!userResponse.ok) {
        throw new AppError(401, "Failed to fetch Discord user");
      }
      ...
      if (!user) {
        user = await prisma.user.create({
          data: {
            ...
            username: `${discordUser.username.toLowerCase()}#${discordUser.discriminator}`,
