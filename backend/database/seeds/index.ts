...
async function main() {
  ...
  const classes = [
    {
      ...
      difficulty: "hard",
      role: "assassin",
      baseHp: 80, baseMana: 40, baseStamina: 120,
    ...
    },
    {
      ...
      difficulty: "easy",
      role: "tank",
      baseHp: 150, baseMana: 30, baseStamina: 80,
    ...
    },
    {
      ...
      difficulty: "medium",
      role: "mage",
      baseHp: 70, baseMana: 100, baseStamina: 50,
    ...
    },
    {
      ...
      difficulty: "easy",
      role: "dps",
      baseHp: 110, baseMana: 20, baseStamina: 110,
    ...
    },
    {
      ...
      difficulty: "medium",
      role: "support",
      baseHp: 90, baseMana: 90, baseStamina: 60,
  ...
  if (crystalGuardian) {
    const skills = [
      { name: "Crystal Shield Bash", description: "Bash the enemy with your crystal shield, dealing damage and stunning.", type: "active", cooldown: 6000, manaCost: 15, staminaCost: 10, rankRequired: 1, sortOrder: 1, baseDamage: 20, damageType: "physical", damageScaling: JSON.stringify({ defense: 0.6 }), debuffsApplied: JSON.stringify([{ buffId: "stun", duration: 1500, stacks: 1 }]) },
      { name: "Fortress Stance", description: "Enter a defensive stance, increasing defense and healing over time.", type: "active", cooldown: 15000, manaCost: 20, staminaCost: 5, rankRequired: 3, sortOrder: 2, baseDamage: 0, damageType: "physical", buffsApplied: JSON.stringify([{ buffId: "fortress", duration: 8000, stacks: 1 }]) },
      { name: "Crystal Eruption", description: "Crystals erupt from the ground, damaging and slowing enemies.", type: "active", cooldown: 10000, manaCost: 25, staminaCost: 15, rankRequired: 5, sortOrder: 3, baseDamage: 45, damageType: "magic", damageScaling: JSON.stringify({ magicDefense: 0.7 }) },
      { name: "Guardian's Sacrifice", description: "Take reduced damage and redirect ally damage to yourself.", type: "active", cooldown: 30000, manaCost: 30, staminaCost: 10, rankRequired: 7, sortOrder: 4, baseDamage: 0, damageType: "physical", buffsApplied: JSON.stringify([{ buffId: "sacrifice", duration: 6000, stacks: 1 }]) },
      { name: "Diamond Shell", description: "Ultimate: Encased in pure diamond, becoming immune to all damage and reflecting attacks.", type: "ultimate", cooldown: 90000, manaCost: 60, staminaCost: 20, rankRequired: 10, sortOrder: 10, baseDamage: 0, damageType: "true", buffsApplied: JSON.stringify([{ buffId: "diamond_shell", duration: 4000, stacks: 1 }]) },
      { name: "Crystal Body", description: "Passively increases max HP and defense.", type: "passive", cooldown: 0, manaCost: 0, staminaCost: 0, rankRequired: 2, sortOrder: 0, baseDamage: 0, damageType: "physical" },
      { name: "Thorns Aura", description: "Passively reflects damage back to attackers.", type: "passive", cooldown: 0, manaCost: 0, staminaCost: 0, rankRequired: 4, sortOrder: 0, baseDamage: 0, damageType: "physical" },
    ];
    for (const skill of skills) {
      await prisma.skill.upsert({
        where: { id: skill.name.toLowerCase().replace(/\s+/g, "-") },
